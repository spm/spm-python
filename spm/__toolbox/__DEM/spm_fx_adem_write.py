from spm.__wrap__ import _Runtime


def spm_fx_adem_write(*args, **kwargs):
  """  returns the flow for a two-joint arm (with action)  
    FORMAT [f]= spm_fx_adem_reach(x,v,a,P)  
     
    x    - hidden states  
      x(1) - joint angle  
      x(2) - joint angle  
      x(3) - angular velocity  
      x(4) - angular velocity  
    v    - exogenous forces (x,y)  
    a    - action (forces) (x,y)  
    P    - parameters  
   __________________________________________________________________________  
  

  [Link to the Matlab implementation.](https://github.com/spm/spm/blob/main/toolbox/DEM/spm_fx_adem_write.m)
  """

  return _Runtime.call("spm_fx_adem_write", *args, **kwargs)
